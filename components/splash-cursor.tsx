...
/**
 * SplashCursor — a lightweight WebGL fluid simulation that follows the cursor.
 * Adapted from the React Bits "Splash Cursor" effect and tuned for a dark
 * background with a deep purple / magenta glowing vapor.
 *
 * Zero runtime dependencies — pure WebGL. The canvas is fixed behind all
 * content (see globals.css / usage) so it never intercepts pointer events or
 * introduces scrollbars.
 *
 * Performance notes:
 * - The simulation pauses entirely when the tab is hidden or the window loses
 *   focus, and when the user prefers reduced motion.
 * - Resolutions are capped and the device pixel ratio is clamped so the effect
 *   stays smooth on laptops and phones without draining the battery.
 */
...
export default function SplashCursor({
  // Lower sim resolution keeps the physics cheap; dye resolution controls the
  // visible smoke detail. Both are conservative for battery friendliness.
  SIM_RESOLUTION = 128,
  DYE_RESOLUTION = 1024,
  ...
  useEffect(() => {
    ...
    // Respect reduced-motion: skip the animated simulation entirely.
    ...
    // Non-null aliases so the many nested closures below type-check cleanly.
    const gl = context.gl
    ...
    function getWebGLContext(canvas: HTMLCanvasElement) {
      ...
      if (!gl) {
        return { gl: null, ext: null as never }
      }
      ...
      if (isWebGL2) {
        ...
        formatRGBA = getSupportedFormat(gl, gl2.RGBA16F, gl.RGBA, halfFloatTexType as number)
        formatRG = getSupportedFormat(gl, gl2.RG16F, gl2.RG, halfFloatTexType as number)
        formatR = getSupportedFormat(gl, gl2.R16F, gl2.RED, halfFloatTexType as number)
      } else {
    ...
    function resizeCanvas() {
      ...
      // Clamp DPR to keep the render buffer light on high-density screens.
    ...
    function splat(x: number, y: number, dx: number, dy: number, color: ColorRGB) {
      ...
      gl.uniform1f(splatProgram.uniforms.aspectRatio, canvasRef.current!.width / canvasRef.current!.height)
    ...
    // ---- Animation loop with visibility / focus gating ----
    let animationId: number
    let running = true

    function loop() {
      if (running) updateFrame()
      animationId = requestAnimationFrame(loop)
    }

    const handleVisibility = () => {
      running = !document.hidden
      // Reset the clock so we don't get one huge dt on resume.
